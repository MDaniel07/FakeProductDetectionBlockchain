...
contract FakeProductDetection {
    ...
    function verifyProduct(
        ...
        if (
            keccak256(bytes(products[productId].manufacturer)) == keccak256(bytes(manufacturer)) &&
            keccak256(bytes(products[productId].model)) == keccak256(bytes(model)) &&
            products[productId].manufacturingDate == manufacturingDate &&
            keccak256(bytes(products[productId].productionLocation)) == keccak256(bytes(productionLocation))
        ) {
            return true;
        }
        return false;
